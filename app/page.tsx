...
const OPS_TEXT = [
  { v: 'equals', l: 'Igual' }, { v: 'not_equals', l: '≠ Diferente' },
  { v: 'contains', l: 'Contém' }, { v: 'not_contains', l: 'Não contém' },
  { v: 'in', l: 'Em (lista)' },
];
const OPS_NUM = [
  { v: 'equals', l: '= Igual' }, { v: 'not_equals', l: '≠ Diferente' },
  { v: 'gt', l: '> Maior' }, { v: 'gte', l: '≥ Maior ou igual' },
  { v: 'lt', l: '< Menor' }, { v: 'lte', l: '≤ Menor ou igual' },
  { v: 'between', l: 'Entre' },
];
const OPS_DATE = [
  { v: 'equals', l: 'Igual' }, { v: 'date_after', l: 'Após' },
  { v: 'date_before', l: 'Antes' }, { v: 'date_between', l: 'Entre' },
];
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/stats').then(r => r.json()).then(d => {
      ...
      if (d.total > 0) {
        ...
        fetchData(d.columns.map((c: ColumnDef) => c.name), [], 1);
      }
    }).catch(() => {});
  ...
  const fetchData = useCallback(async (
    cols?: string[], conds?: FilterCondition[], pg?: number,
    sCol?: string, sDir?: 'asc' | 'desc'
  ) => {
    setLoading(true);
    try {
  ...
  const handleUpload = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (d.success) {
        ...
        setConditions([]); setPage(1);
        fetchData(allCols, [], 1);
      } else { alert(d.error); }
    } catch { clearInterval(iv); alert('Erro no upload'); }
  ...
  const addCondition = () => {
    ...
    setConditions(p => [...p, { column: columns[0].name, operator: 'equals', value: '' }]);
  };
  ...
  const updateCond = (i: number, field: string, val: string) => {
    setConditions(p => {
      ...
      if (field === 'column') { n[i].operator = 'equals'; n[i].value = ''; }
      return n;
    });
  };
  ...
  const applyFilters = () => { setPage(1); fetchData(selectedCols, conditions, 1, sortCol, sortDir); };

  const clearFilters = () => { setConditions([]); setPage(1); fetchData(selectedCols, [], 1, sortCol, sortDir); };
  ...
  const goPage = (p: number) => { setPage(p); fetchData(selectedCols, conditions, p, sortCol, sortDir); };
  ...
  const handleSave = async () => {
    ...
    setShowSave(false); setFilterName(''); setFilterDesc('');
  };
  ...
  const exportFilter = (f: SavedFilter, e: React.MouseEvent) => {
    ...
    a.href = url; a.download = `filtro_${f.name.replace(/\s+/g, '_')}.json`; a.click();
  ...
  const exportAllFilters = () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'todos_filtros.json'; a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const exportCSV = async () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'dados_filtrados.csv'; a.click();
    URL.revokeObjectURL(url);
  };
    ...
    <div className="app">
      ...
      <div className="sidebar">
        <div className="sidebar-header">
          <div className="logo">CSV<span>Filter</span>Pro</div>
        </div>
        ...
        <div className="sidebar-scroll">
          ...
          <div className="section">
            ...
            >
              ...
              <div className="label">{uploading ? 'Processando...' : 'Upload CSV'}</div>
          ...
          {hasData && (
            <>
              ...
              <div className="stats-row">
                <div className="stat">
                  <div className="stat-value">{rowCount.toLocaleString('pt-BR')}</div>
                  <div className="stat-label">Linhas</div>
                </div>
                <div className="stat">
                  <div className="stat-value">{total.toLocaleString('pt-BR')}</div>
              ...
              <div className="section">
                <div className="section-title">
                  ...
                  <div className="col-actions" style={{ padding: 0 }}>
                    <button className="link-btn" onClick={selectAllCols}>Todas</button>
                    <button className="link-btn" onClick={deselectAllCols}>Nenhuma</button>
                  </div>
                </div>
                <div style={{ padding: '0 4px' }}>
                  <input className="col-search" placeholder="Buscar coluna..."
                    value={colSearch} onChange={e => setColSearch(e.target.value)} />
                </div>
                <div className="col-list">
                  {filteredCols.map(c => (
                    <div key={c.name} className={`col-item ${selectedCols.includes(c.name) ? 'on' : ''}`}
                      onClick={() => toggleCol(c.name)}>
                      <div className="col-check">{selectedCols.includes(c.name) && '✓'}</div>
                      <span>{c.label}</span>
              ...
              <div className="section">
                <div className="section-title">
                  ...
                  <button className="btn btn-sm btn-blue" onClick={addCondition}>+ Filtro</button>
                </div>
                <div style={{ padding: '0 4px' }}>
                  {conditions.length === 0 && (
                  ...
                  {conditions.map((c, i) => (
                    <div key={i} className="filter-card">
                      <div className="filter-row">
                        ...
                        <input
                          type={columns.find(x => x.name === c.column)?.type === 'date' ? 'date' : 'text'}
                          placeholder="Valor"
                          value={c.value}
                        ...
                        <button className="btn btn-sm btn-icon btn-red" onClick={() => removeCond(i)}>✕</button>
                      </div>
                    </div>
                  ))}
                  ...
                  {conditions.length > 0 && (
                    <button className="link-btn" style={{ marginTop: 4, display: 'block', textAlign: 'center', width: '100%' }}
              ...
              <div className="section">
                ...
                <div style={{ padding: '0 4px' }}>
                  <div className="btn-group" style={{ marginBottom: 8 }}>
                    <button className="btn btn-sm btn-green" onClick={() => setShowSave(true)}>💾 Salvar Atual</button>
                    <button className="btn btn-sm btn-ghost" onClick={() => filterFileRef.current?.click()}>📥 Importar</button>
                    {savedFilters.length > 0 && (
                      <button className="btn btn-sm btn-ghost" onClick={exportAllFilters}>📤 Exportar Todos</button>
                    )}
                  </div>
                  <input ref={filterFileRef} type="file" accept=".json" hidden
                    onChange={e => { const f = e.target.files?.[0]; if (f) importFilters(f); }} />

                  {savedFilters.map(f => (
                    <div key={f.id} className="saved-item" onClick={() => loadFilter(f)}>
                      <div>
                        <div className="saved-name">{f.name}</div>
                        <div className="saved-meta">
                          {f.conditions.length} filtro(s) • {f.selectedColumns.length} col(s)
                          {f.description && ` • ${f.description}`}
                        </div>
                      </div>
                      <div className="saved-actions">
                        <button className="btn btn-sm btn-icon btn-ghost" onClick={e => exportFilter(f, e)} title="Exportar">📤</button>
                        <button className="btn btn-sm btn-icon btn-red" onClick={e => deleteFilter(f.id, e)} title="Excluir">✕</button>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {!hasData ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div className="empty">
              ...
              <p style={{ fontSize: 11, marginTop: 4 }}>O scraper roda às 06:00, 12:00 e 18:00</p>
            </div>
          </div>
        ) : (
          <>
            <div className="main-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontWeight: 600, color: 'var(--text-0)' }}>Resultados</span>
                <span style={{ color: 'var(--text-3)', fontSize: 12 }}>
                  {total.toLocaleString('pt-BR')} registro(s)
                </span>
              </div>
              <div className="btn-group">
                <button className="btn btn-sm btn-ghost" onClick={exportCSV}>📥 Exportar CSV</button>
              </div>
            </div>
            ...
            {totalPages > 1 && (
              <div className="pagination">
                ...
                <div className="page-btns">
                  <button className="pg-btn" disabled={page <= 1} onClick={() => goPage(1)}>«</button>
                  ...
                  <button className="pg-btn" disabled={page >= totalPages} onClick={() => goPage(totalPages)}>»</button>
                </div>
              </div>
            )}
      ...
      {showSave && (
        <div className="modal-bg" onClick={() => setShowSave(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>Salvar Filtro</h3>
            <input className="modal-input" placeholder="Nome do filtro"
              value={filterName} onChange={e => setFilterName(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleSave()} autoFocus />
            <input className="modal-input" placeholder="Descrição (opcional)"
            ...
            <div className="btn-group" style={{ justifyContent: 'flex-end' }}>
              <button className="btn btn-ghost" onClick={() => setShowSave(false)}>Cancelar</button>
